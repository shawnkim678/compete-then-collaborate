...
**Interpretation (v0.2, addressing peer review).** MBPP near-ceiling scores (99–100%) most likely reflect *benchmark saturation / training exposure* (MBPP is a public 2021 benchmark seen by all three teachers), **not** differential skill; we therefore do **not** rest the teacher ranking on MBPP. On the harder code_contests set the picture differs and separates the teachers more clearly: **Gemini leads (77%, 115/150), followed by Codex (69%) ≈ Claude (67%), then Grok (50%)** — so no single vendor's model is uniquely best, and in particular Claude is *not* the top solver on hard problems. ‡Gemini's competition run was affected by the same class of infrastructure artifact as Grok's: its API prepaid credits were depleted partway through generation, so the final 34/150 problems returned billing errors (429) counted as no-code. After the credits were restored we re-ran exactly those 34 items and merged the results; Gemini's fair pass@1 is 77% (115/150) with 100% code-extraction. Like the Grok correction, this is a *fairness* fix (removing a non-capability artifact), applied identically and reported transparently. †Grok's first-pass competition score was depressed by an **infrastructure artifact** — initially 52% of its batch CLI calls returned empty (timeout/flakiness), not wrong code; a direct call yields a correct fenced solution. We added empty-response retries and re-measured Grok: its **fair** pass@1 is **50% (75/150)** with code-extraction success rising from 48% to 73% (109/150). A residual **27% (41/150) still returned no usable code even after retries**, which we cannot fully attribute to CLI flakiness vs. genuine difficulty; we report it transparently rather than discard those items. The empty-response fix is a *fairness* correction; the remaining Grok–Codex/Claude gap on hard problems (50% vs 67–69%) is thus a conservative (Grok-favorable) estimate. Crucially, the ranking is **not** produced by any LLM judge (execution only), so *self-preference/family bias is excluded by design*; the residual risks are asymmetric train-leak and CLI/parser artifacts, addressed here and in §7.
...
- **The teacher ranking is weaker than the student-side result.** Because of saturation and the above artifacts, "which teacher is best" is only weakly supported on the hard subset (Codex ≈ Claude). By contrast, the **SFT-fails / RLVR-succeeds reversal is robust**: it is a change in the *student's* held-out performance and does not depend on the teacher ranking or on any LLM judge.
