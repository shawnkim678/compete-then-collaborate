...
% Genesis (DGX Spark GB10). Companion research log: AI교수진_코딩교습_비교연구_v1.
...
them (Gemini 77\%~$>$~Codex 69\%~$\approx$~Claude 67\%~$>$~Grok 50\%); still, the most robust
...
(RLVR)} environment instead \textbf{improves} the student (5.9\%$\rightarrow$7.4\% on held-out
competition problems), reversing the direction of SFT. Our central claim is that the value of
...
\bottomrule
\end{tabular}
\caption{Hard competition problems separate the teachers: Gemini~$>$~Codex~$\approx$~Claude~$>$~Grok.
No single vendor is uniquely best; Claude is not the top solver on hard problems.}
\end{table}
...
exposure} (MBPP is a public 2021 benchmark seen by all teachers), \textbf{not} differential
skill; we therefore do \textbf{not} rest the teacher ranking on MBPP. On the harder
\texttt{code\_contests} set the picture differs and separates the teachers more clearly:
\textbf{Gemini leads (77\%, 115/150), then Codex (69\%)~$\approx$~Claude (67\%), then Grok (50\%)},
i.e.\ no single vendor's model is uniquely best and Claude is \emph{not} the top solver on hard
problems. $^{\ddagger}$Gemini's competition run hit the same class of infrastructure artifact as
Grok's: its API prepaid credits were depleted partway through generation, so the final 34/150
problems returned billing errors (429) counted as no-code. After credits were restored we re-ran
exactly those 34 items and merged them; Gemini's fair pass@1 is 77\% (115/150) with 100\%
extraction. Like the Grok correction, this is a \emph{fairness} fix applied identically and
reported transparently. $^{\dagger}$Grok's
first-pass competition score was depressed by an \textbf{infrastructure artifact} --- initially
52\% of its batch CLI calls returned empty (timeout/flakiness), not wrong code; a direct call
yields a correct fenced solution. We added empty-response retries and re-measured Grok: its
\textbf{fair} pass@1 is \textbf{50\% (75/150)}, with code-extraction success rising from 48\% to
73\% (109/150). A residual \textbf{27\% (41/150) still returned no usable code even after
retries}, which we cannot fully attribute to CLI flakiness vs.\ genuine difficulty; we report it
transparently rather than discard those items. The empty-response fix is a \emph{fairness}
correction; the remaining Grok--Codex/Claude gap on hard problems (50\% vs 67--69\%) is thus a
conservative (Grok-favorable) estimate. Crucially, the ranking is \textbf{not} produced by any LLM judge (execution
only), so \emph{self-preference/family bias is excluded by design}; the residual risks are
asymmetric train-leak and CLI/parser artifacts, addressed here and in Section~\ref{sec:limits}.
...
\begin{itemize}
  ...
  \item \textbf{The teacher ranking is weaker than the student-side result.} Because of
    saturation and the above artifacts, ``which teacher is best'' is only weakly supported on the
    hard subset (Codex~$\approx$~Claude). By contrast, the \textbf{SFT-fails / RLVR-succeeds
    reversal is robust}: it is a change in the \emph{student's} held-out performance and does not
    depend on the teacher ranking or on any LLM judge.
