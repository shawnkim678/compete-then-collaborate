...
\section*{재현성}
모든 코드, 태스크뱅크, 히든 테스트, 결과, 그리고 본 논문은 \url{https://github.com/shawnkim678/compete-then-collaborate}에서 공개한다. 실행검증된 태스크뱅크는 Hugging Face 데이터셋으로, 최종 RLVR(GRPO) LoRA 어댑터는 Hugging Face 모델로 함께 배포한다.
\begin{itemize}
  \item 데이터셋: \url{https://huggingface.co/datasets/shawnmkim/compete-collab-taskbank}
  \item 모델: \url{https://huggingface.co/shawnmkim/qwen2.5-coder-7b-rlvr-compete-collab}
\end{itemize}
독자는 보고된 모든 수치를 오프라인으로 재검증하거나(\texttt{python reproduce.py --check-banks --selftest}), 자신의 API 키와 각 공급자의 이용약관에 따라 교사 풀이를 재생성할 수 있다. 우리는 어떤 교사의 원시 출력도 재배포하지 않는다(윤리 진술 참조).
